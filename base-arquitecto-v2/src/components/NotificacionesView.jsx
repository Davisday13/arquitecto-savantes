...
export default function NotificacionesView() {
  ...
  useEffect(() => {
    ...
    supabase.from('notificaciones').select('*').eq('destinatario_id', profile.id).order('created_at', { ascending: false }).limit(100).then(({ data }) => setNotifs(data || []));
  }, [profile?.id]);

  const marcarLeida = async (id) => {
    await supabase.from('notificaciones').update({ estado: 'LEIDA', leida_at: new Date().toISOString() }).eq('id_notificacion', id);
    setNotifs(prev => prev.map(n => n.id_notificacion === id ? { ...n, estado: 'LEIDA', leida_at: new Date().toISOString() } : n));
  };
      ...
      <Card>
        {notifs.length === 0 ? (
          ...
          <div className="space-y-2">
            {notifs.map(n => (
              <div key={n.id_notificacion} className={`flex items-start gap-3 p-3 rounded-lg border ${n.estado === 'PENDIENTE' ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-200'}`}>
                ...
                {n.estado === 'PENDIENTE' && (
                  <button onClick={() => marcarLeida(n.id_notificacion)} className="p-1 hover:bg-green-100 rounded text-green-600" title="Marcar como leída">
